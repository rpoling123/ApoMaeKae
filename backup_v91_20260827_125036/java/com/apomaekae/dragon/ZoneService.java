...
public class ZoneService extends Service implements LocationListener {
    ...
    @Override public void onCreate(){
        ...
        if(Build.VERSION.SDK_INT>=26){
            nm.createNotificationChannel(new NotificationChannel(
                "guard","Zone Guard",NotificationManager.IMPORTANCE_LOW));
        }
        ...
        try{
            ...
            byte[] b=new byte[in.available()]; in.read(b); in.close();
    ...
    @Override public void onLocationChanged(Location l){
        boolean safe=false; String hit="";
        SharedPreferences p=getSharedPreferences("z",0);
        try{
            for(int g=0;g<groups.length();g++){
                JSONObject gr=groups.getJSONObject(g); JSONArray ps=gr.getJSONArray("polygons");
                for(int k=0;k<ps.length();k++){
                    String prefKey="z_"+g+"_"+k;
                    boolean selected=p.getBoolean(prefKey,false);
                    if(!p.contains(prefKey) && p.getBoolean("g"+g,true)) selected=true;
                    if(!selected) continue;
                    JSONObject poly=ps.getJSONObject(k); JSONArray a=poly.getJSONArray("points");
                    if(insideOrNear(l.getLatitude(),l.getLongitude(),a,500)){
                        safe=true; hit=poly.optString("name",gr.optString("name","โซน")); break;
                    }
                }
                if(safe) break;
            }
        ...
        if(!safe && lastSafe) speak("แจ้งเตือน คุณอยู่นอกโซน");
        if(safe && !lastSafe) speak("เข้าโซนแล้ว "+hit);
        if(safe && !hit.equals(lastHit) && !lastHit.isEmpty()) speak("เข้าสู่ "+hit);
        lastSafe=safe; lastHit=hit;
    }

    private void speak(String s){
        try{
            if(tts!=null) tts.speak(s,TextToSpeech.QUEUE_FLUSH,null,"apo_zone_"+System.currentTimeMillis());
        }catch(Exception ignored){}
    }
    ...
    private boolean insideOrNear(double lat,double lon,JSONArray a,double buf)throws Exception{
        ...
        for(int i=0,j=n-1;i<n;j=i++){
            ...
            double dy=(yi-lat)*111320.0; min=Math.min(min,Math.sqrt(dx*dx+dy*dy));
        }
        return c || min<=buf;
    }
