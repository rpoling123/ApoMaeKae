...
const LATEST_VERSION="9.2.4";const PORT=Number(process.env.PORT||8080),TOKEN=process.env.ADMIN_TOKEN||"ApoMaeKae_Admin_2026_X7k9",PAYMENT_SECRET=process.env.PAYMENT_WEBHOOK_SECRET||"";
const DB=path.join(__dirname,"licenses.json"),ORDERS=path.join(__dirname,"orders.json"),PUB=path.join(__dirname,"public");
const PLANS={day:{label:"1 วัน",days:1,price:5},week:{label:"7 วัน",days:7,price:15},month:{label:"30 วัน",days:30,price:39}};
...
function makeKey(o){let db=read(DB,{keys:{}}),k=key(),now=Date.now(),z=now+o.days*86400000;db.keys[k]={key:k,plan:o.plan,planLabel:o.planLabel,price:o.price,startAt:new Date(now).toISOString(),expiresAt:new Date(z).toISOString(),maxDevices:Number(o.maxDevices||1),devices:[],onlineDevices:{},revoked:false,orderId:o.orderId,paymentRef:o.paymentRef||"",appVersion: LATEST_VERSION,createdAt:new Date().toISOString()};write(DB,db);return db.keys[k]}
function onlineCount(k,now){let map=k.onlineDevices&&typeof k.onlineDevices==="object"?k.onlineDevices:{};return Object.values(map).filter(t=>now-Number(t)<90000).length}
function registerDevice(k,deviceId,now){k.devices=Array.isArray(k.devices)?k.devices:[];k.onlineDevices=k.onlineDevices&&typeof k.onlineDevices==="object"?k.onlineDevices:{};if(!deviceId)return {ok:true};if(!k.devices.includes(deviceId)){if(k.devices.length>=Number(k.maxDevices||1))return {ok:false,reason:"device_limit"};k.devices.push(deviceId)}k.onlineDevices[deviceId]=now;return {ok:true}}
...
if(q.method==="GET"&&p==="/api/health")return send(r,200,{ok:true,version:"V9.2.4",serverTime:Date.now()});
if(q.method==="POST"&&p==="/api/license/check"){let db=read(DB,{keys:{}}),k=db.keys[String(b.key||"").trim().toUpperCase()],now=Date.now();let clientVersion=String(b.appVersion||"").trim();if(clientVersion!==LATEST_VERSION)return send(r,200,{active:false,online:false,code:"VERSION_MISMATCH",message:"KEY ใช้ได้เฉพาะเวอร์ชันล่าสุด "+LATEST_VERSION,serverTime:now,latestVersion:LATEST_VERSION,appVersion:clientVersion});if(!k)return send(r,404,{active:false,message:"ไม่พบ Key",serverTime:now});let z=Date.parse(k.expiresAt);if(String(k.appVersion||"")!==LATEST_VERSION)return send(r,200,{active:false,online:false,code:"KEY_VERSION_OLD",message:"KEY นี้เป็นของเวอร์ชันเก่า",serverTime:now,expiresAt:z,keyVersion:k.appVersion||"",latestVersion:LATEST_VERSION});if(k.revoked)return send(r,200,{active:false,message:"Key ถูกระงับ",serverTime:now,expiresAt:z});if(now<Date.parse(k.startAt))return send(r,200,{active:false,message:"ยังไม่ถึงวันเริ่มใช้งาน",serverTime:now,expiresAt:z});if(!Number.isFinite(z)||now>=z)return send(r,200,{active:false,online:false,message:"Key หมดอายุแล้ว",serverTime:now,expiresAt:z});let reg=registerDevice(k,String(b.deviceId||"").trim(),now);if(!reg.ok)return send(r,200,{active:false,online:false,message:"KEY ใช้ครบจำนวนเครื่องแล้ว",serverTime:now,expiresAt:z,maxDevices:Number(k.maxDevices||1),used:k.devices.length,onlineCount:onlineCount(k,now),free:0});write(DB,db);return send(r,200,{active:true,online:true,version:"V9.2.4",serverTime:now,expiresAt:z,plan:k.planLabel,maxDevices:Number(k.maxDevices||1),used:k.devices.length,onlineCount:onlineCount(k,now),free:Math.max(0,Number(k.maxDevices||1)-k.devices.length)})}
if(q.method==="POST"&&p==="/api/license/heartbeat"){let db=read(DB,{keys:{}}),k=db.keys[String(b.key||"").trim().toUpperCase()],now=Date.now();if(!k)return send(r,404,{online:false,message:"ไม่พบ Key",serverTime:now});let z=Date.parse(k.expiresAt);if(k.revoked)return send(r,200,{online:false,expired:false,message:"Key ถูกระงับ",serverTime:now,expiresAt:z});if(!Number.isFinite(z)||now>=z)return send(r,200,{online:false,expired:true,message:"Key หมดอายุแล้ว",serverTime:now,expiresAt:z});let reg=registerDevice(k,String(b.deviceId||"").trim(),now);if(!reg.ok)return send(r,200,{online:false,message:"KEY ใช้ครบจำนวนเครื่องแล้ว",serverTime:now,expiresAt:z,maxDevices:Number(k.maxDevices||1),used:k.devices.length,onlineCount:onlineCount(k,now),free:0});write(DB,db);return send(r,200,{online:true,expired:false,serverTime:now,expiresAt:z,maxDevices:Number(k.maxDevices||1),used:k.devices.length,onlineCount:onlineCount(k,now),free:Math.max(0,Number(k.maxDevices||1)-k.devices.length)})}
if(q.method==="GET"&&p==="/api/license/online"){let keyId=String(u.searchParams.get("key")||"").trim().toUpperCase(),db=read(DB,{keys:{}}),k=db.keys[keyId],now=Date.now();if(!k)return send(r,404,{online:false,message:"ไม่พบ Key",serverTime:now});let z=Date.parse(k.expiresAt),expired=!!k.revoked||!Number.isFinite(z)||now>=z,oc=expired?0:onlineCount(k,now);return send(r,200,{online:!expired&&oc>0,expired,serverTime:now,expiresAt:z,maxDevices:Number(k.maxDevices||1),used:Array.isArray(k.devices)?k.devices.length:0,onlineCount:oc,free:Math.max(0,Number(k.maxDevices||1)-(Array.isArray(k.devices)?k.devices.length:0))})}
if(q.method==="POST"&&p==="/api/order/create"){let pl=PLANS[b.plan];if(!pl)return send(r,400,{error:"แพ็กเกจไม่ถูกต้อง"});let id="ORD-"+Date.now().toString(36).toUpperCase()+"-"+crypto.randomBytes(2).toString("hex").toUpperCase(),db=read(ORDERS,{orders:{}});db.orders[id]={orderId:id,plan:b.plan,planLabel:pl.label,days:pl.days,price:pl.price,status:"pending",createdAt:new Date().toISOString(),paymentRef:"",key:"",expiresAt:""};write(ORDERS,db);return send(r,200,pub(db.orders[id]))}
...
let db=read(DB,{keys:{}});
let od=read(ORDERS,{orders:{}});

if(p.startsWith("/api/admin/") && !(q.method==="POST" && (p==="/api/admin/keys" || p==="/api/admin/keys/update" || p==="/api/admin/keys/delete"))){if(!auth(q))return send(r,401,{error:"unauthorized"});
